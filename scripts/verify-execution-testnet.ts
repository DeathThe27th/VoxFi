...
import { resolveAndQuote } from "../src/lib/execution";
...
async function main(){
  ...
  const actions=await Promise.all(plan.actions.map(action=>resolveAndQuote(action,testnetDeployments.sessionAccount)));
  const results=await executeApprovedPlan(plan,crypto.randomUUID(),"testnet-verifier",testnetDeployments.sessionAccount,actions);console.log("EXECUTION_ENGINE_EVIDENCE",JSON.stringify({configureHash,transactionHashes:results.map(r=>r.hash)}));
}
