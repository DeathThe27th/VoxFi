...
export async function executeApprovedPlan(plan:Plan,conversationId:string,ownerId:string,smartAccount:`0x${string}`,confirmedActions:ResolvedAction[]){
  ...
  if(confirmedActions.length!==plan.actions.length)throw new Error("Confirmed quote does not match the plan");
  const completed:Array<{action:ResolvedAction;hash:`0x${string}`}> = [];
  for(const action of confirmedActions){if(action.type!=="swap")throw new Error("Transfers require owner-wallet authorization in this MVP");if(action.expiresAt<=Date.now())throw new Error("The confirmed quote expired. Ask Vox to quote the plan again.");if(action.target.toLowerCase()!==allowedTarget.toLowerCase()||action.calldata.slice(0,10).toLowerCase()!==allowedSelector.toLowerCase())throw new Error("Smart-session policy rejected this route");
    ...
    const request=(await client.simulateContract({account:relayer,address:smartAccount,abi,functionName:"executeSession",args:[action.target,0n,action.calldata,Number(sig.v),sig.r,sig.s]})).request;const hash=await wallet.writeContract(request);const receipt=await client.waitForTransactionReceipt({hash});if(receipt.status!=="success")throw new Error(`Transaction reverted: ${hash}`);completed.push({action,hash});addActivity({id:crypto.randomUUID(),ownerId,conversationId,type:"swap",summary:`${action.amountIn} ${action.tokenIn} → ${action.expectedOut} ${action.tokenOut}`,status:"confirmed",timestamp:new Date().toISOString(),transactionHash:hash});
  }return completed;
}
