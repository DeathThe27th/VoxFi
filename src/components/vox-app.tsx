...
function Landing({login}:{login:()=>void}){return <main className="landing"><nav><div className="wordmark">vox<span>.</span></div><button onClick={login}>Sign in</button></nav><section className="hero"><div className="eyebrow">VOICE-NATIVE FINANCE · X LAYER</div><h1>Say what you want.<br/><em>Vox handles the intent.</em></h1><p>Ask for balances and build testnet transaction plans through natural conversation. Deterministic code validates every financial action.</p><button className="hero-cta" onClick={login}>Start with email</button><small>Email, Google, or wallet · X Layer Testnet only</small></section><section className="proof"><article><b>01</b><h2>Speak naturally</h2><p>No command syntax. Vox transcribes and understands the financial intent.</p></article><article><b>02</b><h2>Review the plan</h2><p>See assets, amounts, quotes, and authorization requirements before execution.</p></article><article><b>03</b><h2>Stay in control</h2><p>Ambiguous requests are clarified. Unsupported actions are rejected.</p></article></section></main>}
