...
export function VoxApp() {
  ...
  const walletAddress=wallets.find(wallet=>wallet.type==="ethereum"&&wallet.walletClientType==="privy")?.address??wallets.find(wallet=>wallet.type==="ethereum")?.address??"";
  const accountAddress=session?.smartAccount??"";
  useEffect(()=>{if("serviceWorker" in navigator)navigator.serviceWorker.register("/sw.js").catch(()=>undefined)},[]);
  useEffect(()=>{if(!authenticated)return;let cancelled=false;fetch("/api/session/setup").then(async res=>{const data=await res.json();if(!res.ok)throw new Error(data.error??"The Vox test session is unavailable");return data as SessionStatus;}).then(data=>{if(!cancelled){setSession(data);setSessionActive(!data.revoked&&data.expiresAt*1000>Date.now());setSessionError("");}}).catch((e:unknown)=>{if(!cancelled){setSessionError(e instanceof Error?e.message:"The Vox test session is unavailable");setSessionActive(false);}});return()=>{cancelled=true;};},[authenticated]);
  ...
  return <main className="shell">
    ...
    <section className="content">
      {tab === "home" && <HomeScreen listening={listening} busy={busy} reply={reply} error={error||sessionError} portfolio={portfolio} walletReady={walletsReady} accountAddress={accountAddress} session={session} sessionActive={sessionActive} onMic={toggleRecording}/>}
      {tab === "activity" && <Activity/>}
      {tab === "settings" && <Settings owner={walletAddress} session={session} sessionActive={sessionActive} sessionError={sessionError}/>}
    </section>
...
function HomeScreen({listening,busy,reply,error,portfolio,walletReady,accountAddress,session,sessionActive,onMic}:{listening:boolean;busy:boolean;reply:VoiceReply|null;error:string;portfolio:Portfolio|null;walletReady:boolean;accountAddress:string;session:SessionStatus|null;sessionActive:boolean;onMic:()=>void}) {
  return <div className="home-screen">
    ...
    {reply && <div className="reply"><div className="reply-label">VOX UNDERSTOOD</div>{reply.transcript&&<small className="transcript">“{reply.transcript}”</small>}<p>{reply.speak}</p>{reply.plan && <Plan plan={reply.plan} resolved={reply.resolvedActions}/>} {reply.ownerActions?.some(a=>a.ownerTransaction)&&<button className="owner-approve" onClick={async()=>{try{if(!window.ethereum)throw new Error("Connect the owner wallet in a browser wallet first");const accounts=await window.ethereum.request({method:"eth_requestAccounts"}) as string[];if(!accounts[0])throw new Error("No owner wallet was selected");for(const action of reply.ownerActions??[])if(action.ownerTransaction)await window.ethereum.request({method:"eth_sendTransaction",params:[{from:accounts[0],...action.ownerTransaction}]});}catch(e){alert(e instanceof Error?e.message:"Owner approval failed");}}}>Approve with owner wallet</button>}</div>}
...
function Activity(){const[items,setItems]=useState<Array<{id:string;summary:string;status:string;timestamp:string;transactionHash?:string}>>([]);const[loading,setLoading]=useState(true);useEffect(()=>{let cancelled=false;fetch("/api/activity").then(r=>r.json()).then(data=>{if(!cancelled)setItems(data.activity??[]);}).catch(()=>undefined).finally(()=>{if(!cancelled)setLoading(false);});return()=>{cancelled=true;};},[]);return <div className="page"><div className="eyebrow">YOUR HISTORY</div><h1>Activity</h1>{loading?<div className="empty"><p>Loading activity…</p></div>:items.length?<div className="activity-list">{items.map(item=><a key={item.id} href={item.transactionHash?`/api/transaction?hash=${item.transactionHash}`:undefined}><span className="status-dot"/><div><strong>{item.summary}</strong><small>{new Date(item.timestamp).toLocaleString()} · {item.status}</small></div><Arrow/></a>)}</div>:<div className="empty"><Clock size={28}/><h2>No activity yet</h2><p>Completed testnet swaps will appear here.</p></div>}</div>}

function Settings({owner,session,sessionActive,sessionError}:{owner:string;session:SessionStatus|null;sessionActive:boolean;sessionError:string}) {const expires=session?new Date(session.expiresAt*1000):null;return <div className="page settings"><div className="eyebrow">ACCOUNT</div><h1>Settings</h1><section><h2>Voice</h2><Setting title="Spoken responses" detail="Enabled through browser speech"/><Setting title="Language" detail="Detected from each request"/></section><section><h2>Network</h2><Setting title="Environment" detail="X Layer Testnet"/><Setting title="Supported reads" detail="OKB, tETH, and tUSDC balances"/></section><section><h2>Wallet</h2><Setting title="Authenticated wallet" detail={owner?`${owner.slice(0,8)}…${owner.slice(-6)}`:"Creating embedded wallet…"}/><Setting title="Demo execution wallet" detail={session?`${session.smartAccount.slice(0,8)}…${session.smartAccount.slice(-6)}`:"Unavailable"}/><Setting title="Voice session" detail={sessionActive?`Active until ${expires!.toLocaleString()}`:sessionError||"Needs owner authorization"}/>{!sessionActive&&<SetupSessionButton/>}</section><p className="fine">This is a shared X Layer testnet demo wallet. It only holds test assets and can only swap through the fixed Vox test AMM.</p></div>; }
function SetupSessionButton(){const[busy,setBusy]=useState(false);const[error,setError]=useState("");async function setup(){setBusy(true);setError("");try{if(!window.ethereum)throw new Error("Connect the demo wallet owner in a browser wallet first");const setup=await fetch("/api/session/setup").then(async res=>{const data=await res.json();if(!res.ok)throw new Error(data.error??"Unable to prepare the session");return data as {transactions:Array<{to:string;data:string}>};});const accounts=await window.ethereum.request({method:"eth_requestAccounts"}) as string[];if(!accounts[0])throw new Error("No owner wallet was selected");for(const transaction of setup.transactions)await window.ethereum.request({method:"eth_sendTransaction",params:[{from:accounts[0],to:transaction.to,data:transaction.data}]});window.location.reload();}catch(e){setError(e instanceof Error?e.message:"Session authorization failed");setBusy(false);}}return <><button className="owner-approve" onClick={setup} disabled={busy}>{busy?"Waiting for owner approval…":"Authorize demo voice session"}</button>{error&&<div className="error">{error}</div>}</>}
