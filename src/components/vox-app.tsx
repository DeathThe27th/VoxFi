...
export function VoxApp() {
  ...
  async function setup(){setSetupBusy(true);setError("");try{if(!owner)await connect();const config=await fetch("/api/session/setup").then(r=>r.json());const activeOwner=owner||localStorage.getItem("voxOwner")||"";if(config.owner?.toLowerCase()!==activeOwner.toLowerCase())throw new Error("For this deployed demo, connect the disposable owner wallet configured during setup.");await fetch("/api/session/setup",{method:"POST"}).then(async r=>{if(!r.ok)throw new Error((await r.json()).error)});for(const tx of config.transactions){const hash=await window.ethereum!.request({method:"eth_sendTransaction",params:[{from:activeOwner,to:tx.to,data:tx.data,value:"0x0"}]}) as string;for(let i=0;i<30;i++){const receipt=await window.ethereum!.request({method:"eth_getTransactionReceipt",params:[hash]});if(receipt)break;await new Promise(r=>setTimeout(r,1000));}}localStorage.setItem("voxReady","true");setReady(true);}catch(e){setError(e instanceof Error?e.message:"Setup failed")}finally{setSetupBusy(false)}}
  ...
  if(!ready)return <Onboarding owner={owner} busy={setupBusy} error={error} connect={async()=>{try{await connect()}catch(e){setError(e instanceof Error?e.message:"Connection failed")}}} setup={setup}/>;
...
function Onboarding({owner,busy,error,connect,setup}:{owner:string;busy:boolean;error:string;connect:()=>void;setup:()=>void}){return <main className="shell onboarding"><div className="wordmark">vox<span>.</span></div><div className="onboarding-copy"><div className="eyebrow">WELCOME TO VOX</div><h1>Finance, at the speed of speech.</h1><p>Connect your owner wallet once, authorize a limited testnet session, then speak naturally to act onchain.</p></div><div className="setup-list"><div className={owner?"done":"current"}><b>1</b><span><strong>Connect owner wallet</strong><small>{owner?`${owner.slice(0,6)}…${owner.slice(-4)}`:"X Layer Testnet"}</small></span></div><div className={owner?"current":""}><b>2</b><span><strong>Configure Vox account</strong><small>Approve test assets and a 24-hour session</small></span></div><div><b>3</b><span><strong>Speak with Vox</strong><small>Every action still needs confirmation</small></span></div></div>{error&&<div className="error">{error}</div>}<button className="primary" disabled={busy} onClick={owner?setup:connect}>{busy?"Configuring on X Layer…":owner?"Authorize Vox":"Connect wallet"}</button><p className="disclaimer">Voice confirmation uses a previously delegated session permission. It is not itself a cryptographic signature.</p></main>}
